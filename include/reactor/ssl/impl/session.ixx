#if not defined BEASTHTTP_REACTOR_SSL_SESSION_IXX
#define BEASTHTTP_REACTOR_SSL_SESSION_IXX
...
::connection_type&
session<BEASTHTTP_REACTOR_SSL_SESSION_TMPL_ATTRIBUTES>::flesh::getConnection()
{
    return connection_;
}
...
BEASTHTTP_REACTOR_SSL_SESSION_TMPL_DECLARE
void
session<BEASTHTTP_REACTOR_SSL_SESSION_TMPL_ATTRIBUTES>::flesh::process_request()
{
...
{
    buffer_type buffer;
    return handshake(ctx, std::move(socket), time_point, resource_map,
...
#endif // not defined BEASTHTTP_REACTOR_SSL_SESSION_IXX
